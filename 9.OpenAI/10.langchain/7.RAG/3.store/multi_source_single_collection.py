...
def bulid_document():
    ...
    for path in FILES:
        ...
        for c in part:
            s.metadata["source"] = os.path.basename(path)
            # 통합된 컬렉션 내에서 문서를 구분하기 위해서 , 각각의 데이터에 메타데이터를 넣음.
            chunks += part
        return Chroma.from_documents(chunks,embeddings, collection_name="unified",
